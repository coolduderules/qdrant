...
def test_incompatible_snapshot_recovery_with_last_active_replica(tmp_path: pathlib.Path):
    ...
    read_peer = bootstrap_peer(tmp_path / "read", 63331, 3)
...
def bootstrap_peer(path: pathlib.Path, port: int, num_peers: int = 1):
    path.mkdir()
    ...
    uris, _, _ = start_cluster(path, num_peers, port_seed = port, extra_env = config)
